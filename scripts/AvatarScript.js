...
Kata.require([
    ...
    FIContent.AvatarScript.prototype.updateAnimation = function(presence, remote){
		 ...
		 var is_turning =  (angspeed != 0);

		var cur_anim = remote.cur_anim;
        this.new_anim = (is_mobile ? "walk" : (is_turning ? "turn" : "idle"));

        if (cur_anim != this.new_anim) {
			
            this.animate(presence, remote, this.new_anim);
            remote.cur_anim = this.new_anim;
        }
    };
